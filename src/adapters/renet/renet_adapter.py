"""Main RE-Net adapter providing a clean interface to RE-Net functionality."""
...
class RENetAdapter:
    ...
    def prepare_dataset(self, preprocess_directory: str, dataset_name: str) -> bool:
        ...
        try:
            ...
            candidate = Path(self.config.data_config.data_directory) / "get_history_graph.py"
            if not candidate.exists():
                self.logger.error(f"Required get_history_graph.py not found in data directory: {candidate}")
                return False
            ...
            if not copy_res.success:
                self.logger.error("Failed to copy get_history_graph.py to RE-Net data directory")
                return False
    ...
    def train(
        ...
        """Run pretraining followed by training as a single step."""
        self.logger.info(
            f"Pretraining: dropout={dropout}, n_hidden={n_hidden}, lr={learning_rate}, epochs={pretrain_epochs}, batch_size={batch_size}"
        )
        pre_success, pre_result, pre_err = self._execute_process_call(
            phase="pretraining",
            call=self.process_manager.pretrain_model,
        ...
        if not pre_success or pre_result is None:
            return TrainingResult(success=False, execution_time=pre_result.execution_time if pre_result else 0.0,
                                  error_message=pre_err)

        self.logger.info(
            f"Training: dropout={dropout}, n_hidden={n_hidden}, lr={learning_rate}, epochs={train_epochs}, batch_size={batch_size}"
        )
        tr_success, tr_result, tr_err = self._execute_process_call(
            phase="training",
            call=self.process_manager.train_model,
            dataset_name=dataset_name,
            dropout=dropout,
            n_hidden=n_hidden,
            learning_rate=learning_rate,
        ...
        if not tr_success or tr_result is None:
            return TrainingResult(success=False, execution_time=tr_result.execution_time if tr_result else 0.0,
                                  error_message=tr_err)

        total_time = pre_result.execution_time + tr_result.execution_time
        return TrainingResult(success=True, execution_time=total_time)
    ...
    def run_testing(self, dataset_name: str, n_hidden: int, run_id: str, results_directory: str) -> TestingResult:
        ...
        success, result, error_message = self._execute_process_call(
            phase="testing",
            call=self.process_manager.test_model,
            dataset_name=dataset_name,
            n_hidden=n_hidden,
            gpu_device=self.config.gpu_device,
            num_k=self.config.num_k_parameter
        ...
        if success and result is not None:
            output_file = self._handle_test_outputs(dataset_name, results_directory, run_id)
            if not output_file:
                self.logger.error("Testing completed but no prediction file was produced")
                return TestingResult(
                    success=False,
                    execution_time=result.execution_time,
                    error_message="Prediction file missing"
                )
            return TestingResult(
                success=True,
    ...
    def _handle_test_outputs(self, dataset_name: str, results_directory: str, run_id: str) -> Optional[str]:
        ...
        Returns:
            Path to the moved prediction output file, or None on failure
        """
        try:
    ...
    def _execute_process_call(self, phase: str, call: Callable[..., Any], **kwargs: Any):
        """Execute a process manager call with consistent error handling."""
        try:
            result: ProcessResult = call(**kwargs)
            if result.succeeded:
                self.logger.info(f"{phase.capitalize()} completed in {result.execution_time:.2f}s")
                return True, result, None
            self.logger.error(f"{phase.capitalize()} failed: {result.stderr}")
            return False, result, result.stderr
        except ProcessError as e:
            self.logger.error(f"{phase.capitalize()} process failed: {e}")
            return False, None, str(e)
        except Exception as e:
            self.logger.error(f"Unexpected error during {phase}: {e}")
            return False, None, str(e)
