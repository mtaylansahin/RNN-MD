...
class ExperimentConfig:
    ...
    model_type: int = 3  # RGCN aggregator
    maxpool: int = 1
    ...
    def _validate_system_config(self) -> None:
        ...
        if self.sequence_length <= 0:
            raise ValueError(f"Sequence length {self.sequence_length} must be positive")
    ...
    @property
    def renet_data_directory(self) -> str:
        """Get the RE-Net data directory path."""
        return os.path.join(self.renet_directory, "data", self.experiment_name)
    
    @property
    def model_save_directory(self) -> str:
        """Get the model save directory path."""
        return os.path.join(self.models_directory, self.experiment_name)
